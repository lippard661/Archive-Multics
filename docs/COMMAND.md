...
| Duplicate component names (joined or crafted archives) | `t` lists all; `d` deletes all; `r` replaces all with the same segment; `x` extracts each in turn to the same entry; `xd`/`xdf` delete every copy from the archive, losing all but the last one extracted (bug 37) | Reading warns; `t`, `d`, `x` as Multics; `xd`/`xdf` delete only the component whose file survives; `r`/`u` refuse the name | Nothing is lost; repeated `xdf` (or `xd` answering no) takes the copies out one at a time. |
